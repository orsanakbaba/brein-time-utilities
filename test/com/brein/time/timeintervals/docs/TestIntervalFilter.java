...
public class TestIntervalFilter {
    ...
    public void testExampleOnIntervalFilter() {
        ...
        Assert.assertTrue(tree.contains(new IdInterval<>("ID1", new LongInterval(1L, 5L))));  // -> true
        Assert.assertTrue(tree.contains(new IdInterval<>("ID2", new LongInterval(1L, 5L))));  // -> true
        Assert.assertFalse(tree.contains(new IdInterval<>("ID3", new LongInterval(1L, 5L)))); // -> false

        // first part of the example
        Assert.assertEquals(
                Collections.singletonList(new IdInterval<>("ID1", new LongInterval(1L, 5L))),
                tree.find(new IdInterval<>("ID1", new LongInterval(1L, 5L)))); // -> [ID1@[1, 5]]
        Assert.assertEquals(
                Collections.singletonList(new IdInterval<>("ID2", new LongInterval(1L, 5L))),
                tree.find(new IdInterval<>("ID2", new LongInterval(1L, 5L)))); // -> [ID2@[1, 5]]
        Assert.assertEquals(
        ...
        Assert.assertEquals(
                Arrays.asList(new IdInterval<>("ID2", new LongInterval(1L, 5L)),
                        new IdInterval<>("ID1", new LongInterval(1L, 5L))),
        ...
        Assert.assertEquals(
                Arrays.asList(new IdInterval<>("ID2", new LongInterval(1L, 5L)),
                        new IdInterval<>("ID1", new LongInterval(1L, 5L))),
                tree.find(new IdInterval<>("ID1", new LongInterval(1L, 5L)), IntervalFilters::interval)); // -> []
    }
}
